...
def export_shards_parallel(
    *,
    ...
    def flush(current_split: str) -> None:
        ...
        # Only cap TRAIN shards. Validation shard does not count against the cap.
        if (
            current_split == "train"
            and max_train_shards is not None
            and stats["files_train"] >= max_train_shards
        ):
            stop_requested = True
            return
    ...
    if stop_requested:
        ...
        print(f"Reached max_train_shards={max_train_shards}, stopping early.", flush=True)
    else:
        flush(split)
...
def parse_args():
    p = argparse.ArgumentParser(description="Export FineWeb docs into custom DP-tokenized shards (multiprocess, v3)")
    p.add_argument(
        "--docs-jsonl",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cap on number of TRAIN shards to write (validation shard does not count)",
    )
    return p.parse_args()
