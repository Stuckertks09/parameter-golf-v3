...
OUTPUT_DIR = "analysis/grid_vocabs_v2"
...
# GRID CONFIG (v2)
# -----------------------------
BASELINE_KEEP_OPTIONS = [650, 680, 700]
PHRASE_KEEP_OPTIONS = [28, 36, 44]
WORD_KEEP_OPTIONS = [24, 32, 40]

BASELINE_SORT_OPTIONS = ["priority"]
PHRASE_SORT_OPTIONS = ["combined"]
WORD_SORT_OPTIONS = ["combined"]

PIN_MODE_OPTIONS = ["none"]
MIN_SUBWORD_SCORE_OPTIONS = [10, 12, 14]
...
def pin_set_for_mode(pin_mode: str):
    if pin_mode == "none":
        return set()
    if pin_mode == "all":
...
def phrase_combined_value(row, active_pins):
...
def word_combined_value(row, active_pins):
    freq = float(row.get("freq", 0))
    score = float(row.get("score", 0))
    cls = row.get("class", "")
    piece = row.get("text", "")
...
def phrase_sort_key(row, mode, active_pins):
    freq = float(row.get("freq", 0))
    score = float(row.get("score", 0))
    piece = row.get("text", "")
    if mode == "freq":
    ...
    if mode == "combined":
        return (-phrase_combined_value(row, active_pins), len(piece), piece)
    raise ValueError(f"Unknown phrase sort mode: {mode}")


def word_sort_key(row, mode, active_pins):
    freq = float(row.get("freq", 0))
    score = float(row.get("score", 0))
    piece = row.get("text", "")
    if mode == "freq":
    ...
    if mode == "score":
        return (-score, len(piece), piece)
    if mode == "combined":
        return (-word_combined_value(row, active_pins), len(piece), piece)
...
def build_vocab(
    ...
    final_rows = []
    used = set()
    active_pins = pin_set_for_mode(pin_mode)
    ...
    for piece in sorted(active_pins):
        ...
        if cls in ALLOWED_PHRASE_CLASSES and phrase_ok(piece):
            if add_piece(
                piece,
                "phrase_merge",
                phrase_combined_value(row, active_pins) + 1_000_000,
                meta={
                    "class": cls,
                    "freq": row.get("freq", 0),
            ...
            if add_piece(
                piece,
                "word_subword_merge",
                word_combined_value(row, active_pins) + 1_000_000,
                meta={
                    "class": cls,
                    "freq": row.get("freq", 0),
                    "score": row.get("score", 0),
    ...
    phrase_candidates.sort(key=lambda r: phrase_sort_key(r, phrase_sort_mode, active_pins))
    ...
    for row in phrase_candidates:
        if add_piece(
            row["text"],
            "phrase_merge",
            phrase_combined_value(row, active_pins),
            meta={
                "class": row.get("class"),
                "freq": row.get("freq", 0),
                "score": row.get("score", 0),
    ...
    word_candidates.sort(key=lambda r: word_sort_key(r, word_sort_mode, active_pins))
    ...
    for row in word_candidates:
        if add_piece(
            row["text"],
            "word_subword_merge",
            word_combined_value(row, active_pins),
            meta={
                "class": row.get("class"),
                "freq": row.get("freq", 0),
                "score": row.get("score", 0),
...
def main():
    ...
    print(f"Generating {len(combos)} vocab variants...")
    ...
    for i, (
        ...
        tag = (
            ...
            f"_p{phrase_keep}"
            f"_w{word_keep}"
            f"_bs-{baseline_sort_mode}"
            f"_ps-{phrase_sort_mode}"
            f"_ws-{word_sort_mode}"
            f"_pin-{pin_mode}"
            f"_ms{min_subword_score}"
        )
        ...
        print(f"[{i}/{len(combos)}] wrote {out_path}")
