...
class FastCustomDPTokenizer:
    ...
    def __init__(self, vocab_rows: list[dict]):
        ...
        # First-char candidate pruning:
        # candidates_by_first_char[ch] -> list[(piece, pid, len(piece))]
        # sorted by length descending for better tie-break behavior and earlier wins
    ...
    def encode_dp(self, text: str) -> list[int]:
        ...
        for i in range(n - 1, -1, -1):
            ...
            if candidates:
                remaining = n - i
                # candidates already sorted by length desc
...
def export_shards_parallel(
    *,
    ...
    # keep input order even with parallel work
    with ProcessPoolExecutor(
        max_workers=workers,
        initializer=_worker_init,
        initargs=(vocab_rows, bos_id, eos_id, append_eos),
    ) as ex:
        # executor.map preserves order of submitted batches
        for tokenized_batch in ex.map(_tokenize_batch, batched_docs(docs_jsonl, batch_docs), chunksize=1):
            for doc_idx, ids_np in tokenized_batch:
                ...
                while pos < len(ids_np):
                    take = min(shard_size_tokens - fill, len(ids_np) - pos)
