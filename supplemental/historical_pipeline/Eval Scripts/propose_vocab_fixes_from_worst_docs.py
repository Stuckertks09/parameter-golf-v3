...
def is_low_information(piece: str) -> bool:
    ...
    if is_probable_proper_noun(piece):
        return True
    # reject long doc-specific snippets unless they are clearly structural
...
def generate_candidates_from_text(text: str, existing: set[str], min_len: int, max_len: int) -> Counter:
    ...
    for line in lines:
        ...
        # newline-prefixed structural spans only
        ...
        # known reusable prose-ish but still valuable connectors
        for n in range(2, min(4, len(line_tokens)) + 1):
            cand = " ".join(line_tokens[:n])
            if cand in {"is the", "with a", "have been", "into the", "through the", "of the", "and the"}:
                add_candidate(counts, cand, 6, existing, min_len, max_len)
...
def apply_swaps(vocab_rows: list[dict], add_scores: list[CandidateScore], remove_rows: list[dict], num_swaps: int):
    ...
    for add, rem in zip(add_scores, remove_rows, strict=True):
        ...
        rows[rid]["source"] = "worst_docs_fix_v4"
        rows[rid]["note"] = {
            "replaced": old_piece,
...
def main() -> None:
    ...
    add_csv = out_dir / f"worst_docs_add_candidates_{ts}.csv"
    remove_csv = out_dir / f"worst_docs_remove_candidates_{ts}.csv"
    swaps_csv = out_dir / f"worst_docs_vocab_swaps_{ts}.csv"
    report_json = out_dir / f"worst_docs_vocab_report_{ts}.json"
    vocab_out = out_dir / f"vocab_best_worstdocs_fix_{ts}.jsonl"
